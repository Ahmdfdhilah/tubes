...
class NPC(AnimatedSprite):
    ...
    def movement(self):
        ...
        if next_pos not in self.game.object_handler.npc_pos:
            angle = math.atan2(next_y + .25 - self.y, next_x + .25 - self.x)
    ...
    def check_hit_in_npc(self):
        if self.ray_cast_value and self.game.player.shot:
            if HALF_WITDH - self.sprite_half_width < self.screen_x < HALF_WITDH + self.sprite_half_width:
                self.game.sound.npc_pain.play()
                self.game.player.shot = False
                self.pain = True
                self.hp -= self.game.weapon.damage
                self.check_health()
